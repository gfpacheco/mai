...
from api.models.models import DocumentChunk, DocumentChunkMetadata, DocumentChunkWithScore, DocumentMetadataFilter, Query, QueryResult, QueryWithEmbedding
...
RESPONSE_MODE = os.environ.get(
    'LLAMA_RESPONSE_MODE', ResponseMode.NO_TEXT.value)
...
def _response_to_query_result(response: Response, query: QueryWithEmbedding) -> QueryResult:
    results = [_source_node_to_doc_chunk_with_score(
    ...
    async def _query(
        ...
        for query in queries:
            if query.filter is not None:
                logger.warning(
                    'Filters are not supported yet, ignoring for now.')
        ...
        if ids is not None:
            for id_ in ids:
                try:
                    ...
                    logger.warning(
                        f'{type(self._index)} does not support delete yet.')
                    return False
